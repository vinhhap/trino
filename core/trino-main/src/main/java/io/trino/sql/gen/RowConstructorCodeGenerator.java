...
import static io.trino.sql.gen.BytecodeSizeEstimator.estimateMaxCodeSize;
        ...
        implements BytecodeGenerator
{
    ...
    // The JVM rejects methods with more than 64KB of bytecode. Stay well below that, and also
    // below HotSpot's HugeMethodLimit (8000 bytes), above which methods are never JIT compiled.
    private static final int MAX_PARTIAL_ROW_CONSTRUCTOR_CODE_SIZE = 8000;
    ...
    private PartialRowConstructor generatePartialRowConstructor(int start, BytecodeGeneratorContext parentContext)
    {
        ...
        while (field < arguments.size()) {
            ...
            int fieldCodeSize = estimateMaxCodeSize(fieldInitialization, scope);
            if (estimatedCodeSize > 0 && estimatedCodeSize + fieldCodeSize > MAX_PARTIAL_ROW_CONSTRUCTOR_CODE_SIZE) {
            ...
            estimatedCodeSize += fieldCodeSize;
            field++;
        }
